...
- 修复：为生时校正消息区增加细宽、透明轨道、低对比圆角拇指，并在 hover 时适度加深；Firefox 使用标准 scrollbar 属性，Chromium / Safari 使用 WebKit 伪元素，颜色继续取现有设计 token。
- 验证：CSS 契约锁定标准与 WebKit 两套样式，聚焦组件测试、目标 ESLint、production build 与生产浏览器 smoke。
- 防复发：独立滚动容器必须复用产品 token，并同时覆盖标准 scrollbar 属性和 WebKit 伪元素；不得引入渐变、重阴影或高饱和装饰。
- 相关记录：BUG-041
- 复发自：无
- 修复版本：本次简约滚动条修复提交
