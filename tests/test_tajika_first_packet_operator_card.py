...
CARD = ROOT / "docs" / "benchmark" / "tajika_steve_jobs_1984_first_packet_operator_card.md"
PACKET = ROOT / "references" / "oracle" / "evidence_packet_templates" / "tajika_steve_jobs_1984_first_packet.json"
...
def test_tajika_first_packet_operator_card_is_actionable() -> None:
    ...
    assert "template_steve_jobs_varshaphala_1984_lahiri" in text
...
def test_tajika_first_packet_template_contains_only_annual_targets() -> None:
    ...
    assert packet["capture_id"] == "external_template_steve_jobs_varshaphala_1984_lahiri"
    assert packet["status"] == "draft"
    assert packet["case_id"] == "template_steve_jobs_varshaphala_1984_lahiri"
    assert packet["metadata"]["source_artifact"] == "references/oracle/artifacts/"
    assert packet["settings"]["target_year"] == 1984
    ...
    assert packet["operator_goal"] == "Fill one external Varshaphala/Tajika annual chart packet."
