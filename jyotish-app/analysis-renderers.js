...
export function renderFunctionalNature(ascSign) {
  ...
  const mkBadges = (list, key) => list.map(p =>
    `<span class="fn-badge ${FN_LABEL[key].cls}">${PLANET_SYMBOLS[p]||''} ${PLANET_CN[p]}</span>`
  ).join('');
  ...
  if (t.yk) html += `<div class="fn-row fn-yk-row"><span class="fn-label">Yogakaraka</span><div class="fn-badges"><span class="fn-badge fn-yk">${PLANET_SYMBOLS[t.yk]||''} ${PLANET_CN[t.yk]}</span></div></div>`;
...
function renderPACItem(key, it) {
  switch(key) {
    case 'P': return `<div class="pac-item pac-pos"><span class="pi-planet">${PLANET_SYMBOLS[it.planet]||''} ${it.pcn}</span><span class="pi-detail">${it.scn} H${it.house} ${it.status||''} ${it.retro?'℞':''} ${it.combust?'[燃]':''}</span><span class="pi-fn fn-${fnCls(it.fn)}">${fnLabel(it.fn)}</span><span class="pi-hr">主管H${it.hr.join('/')}</span></div>`;
    case 'A': return `<div class="pac-item pac-asp"><span class="pi-planet">${PLANET_SYMBOLS[it.planet]||''} ${it.pcn} (H${it.house})</span>${it.received.map(r=>`<span class="pi-recv ${r.impact==='吉'?'recv-good':r.impact==='凶'?'recv-bad':'recv-neut'}">${PLANET_CN[r.from]} ${r.type} → ${r.impact}</span>`).join('')}</div>`;
    case 'C': return `<div class="pac-item pac-conj${it.pollution?' pac-poll':''}"><span class="pi-planets">${it.p1cn} + ${it.p2cn}</span><span class="pi-house">H${it.house}</span>${it.pollution?'<span class="pi-poll-badge">[!] 能量污染</span>':''}<span class="pi-fn fn-${fnCls(it.f1)}">${fnLabel(it.f1)}</span><span class="pi-fn fn-${fnCls(it.f2)}">${fnLabel(it.f2)}</span></div>`;
    case 'D': return `<div class="pac-item pac-dhana"><span class="pi-type">${it.type}</span><span class="pi-lords">${it.lcn}</span><span class="pi-house">H${it.house}</span><span class="pi-str">${it.str}</span></div>`;
    case 'Ar': return `<div class="pac-item pac-arishta"><span class="pi-type">${it.type}</span><span class="pi-desc">${it.desc}</span></div>`;
    case 'R': return `<div class="pac-item pac-raja"><span class="pi-type">${it.type}</span><span class="pi-lords">${it.lcn}</span><span class="pi-house">H${it.house}</span><span class="pi-str">${it.str}</span></div>`;
    case 'E': return `<div class="pac-item pac-exchg"><span class="pi-type">${it.type} (${it.nature})</span><span class="pi-desc">${it.desc}</span></div>`;
    case 'S': return `<div class="pac-item pac-special"><span class="pi-type">${it.type}</span><span class="pi-desc">${it.desc}</span></div>`;
    default: return '';
  }
}
...
export function renderHouseInfluence(matrix) {
  ...
  for (const h of matrix) {
    ...
    html += `<div class="hi-card ${vc}"><div class="hi-header"><span class="hi-num">H${h.house}</span><span class="hi-sign">${h.scn}</span><span class="hi-lord">主: ${h.lcn}</span></div><div class="hi-score ${vc}">${h.total>0?'+':''}${h.total}</div><div class="hi-verdict ${vc}">${h.verdict}</div><div class="hi-items">`;
    for (const it of h.items) {
      const ic = it.sc > 0 ? 'hi-pos' : it.sc < 0 ? 'hi-neg' : 'hi-zero';
      html += `<div class="hi-influence ${ic}"><span class="hi-src">${it.src}</span><span class="hi-sc">${it.sc>0?'+':''}${it.sc}</span></div>`;
    }
    html += `</div></div>`;
  }
  c.innerHTML = html;
...
export function renderVargottama(vgList) {
  ...
  for (const v of vgList) {
    html += `<div class="vg-card"><span class="vg-planet">${PLANET_SYMBOLS[v.planet]||''} ${v.pcn}</span><span class="vg-badge">Vargottama</span><span class="vg-sign">${v.d1}</span></div>`;
  }
  html += '</div>';
...
export function renderFrequency(freq) {
  ...
  for (const [pn, f] of Object.entries(freq)) {
    ...
    html += `<div class="freq-card"><div class="freq-header"><span class="freq-planet">${PLANET_SYMBOLS[pn]||''} ${f.pcn}</span><span class="freq-pct ${lc}">${f.pct}%</span></div><div class="freq-top">${f.topScn} (${f.count}/${16})</div><div class="freq-level ${lc}">${f.level}</div><div class="freq-bar-wrap"><div class="freq-bar" style="width:${f.pct}%"></div></div><div class="freq-dist">${f.dist.map(d=>`${d.scn} ${d.p}%`).join(' · ')}</div></div>`;
  }
  c.innerHTML = html;
}
...
export function renderTriangle(tri) {
  ...
  for (const t of tri) {
    ...
    html += `<tr><td>${PLANET_SYMBOLS[t.planet]||''} ${t.pcn}</td><td>${t.d1sign}</td><td class="${stCls(t.d1st)}">${t.d1st}</td><td>${t.d9sign}</td><td class="${stCls(t.d9st)}">${t.d9st}</td><td>${t.d10sign}</td><td class="${stCls(t.d10st)}">${t.d10st}</td><td>${t.vargottama?'✓':'—'}</td><td class="${vc}">${t.verdict}</td></tr>`;
  }
  html += '</tbody></table>';
  c.innerHTML = html;
}
...
export function renderRamanGrades(scores) {
  ...
  for (const s of scores) {
    ...
    html += `<div class="rg-card ${gc}"><div class="rg-top"><span class="rg-num">H${s.house}</span><span class="rg-sign">${s.scn}</span><span class="rg-grade ${gc}">${s.grade}</span></div><div class="rg-domain">${domain?domain.name:''}</div><div class="rg-scores"><span class="rg-pos">+${s.pos}</span> / <span class="rg-neg">-${s.neg}</span> = <span class="rg-net">${s.net>0?'+':''}${s.net}</span></div><div class="rg-lord">宫主: ${s.lcn}</div><div class="rg-details">`;
    for (const d of s.details) html += `<div class="rg-detail ${d.startsWith('✓')?'rd-pos':d.startsWith('✗')?'rd-neg':'rd-neut'}">${d}</div>`;
    html += '</div></div>';
  }
  c.innerHTML = html;
}
...
export function renderTrikonaKendra(data) {
  ...
  for (const h of keyHouses) {
    ...
    html += `<div class="tk-lord-card">
      ...
      <div class="tk-lord-name">${hl.lord_cn} (${hl.lord})</div>
      <div class="tk-lord-in">→ H${hl.lordInHouse || '?'} ${hl.lordStatus || ''} ${hl.lordRetro ? '℞' : ''}</div>
    </div>`;
  }
  html += '</div></div>';
  ...
  if (trikonaConnections.length > 0) {
    ...
    for (const conn of trikonaConnections) {
      ...
      html += `<div class="tk-conn ${qClass}"><span class="tk-conn-icon">${conn.quality === 'excellent' ? '★' : conn.quality === 'strong' ? '◆' : '→'}</span> ${conn.desc}</div>`;
    }
    html += '</div></div>';
  } else {
  ...
  if (crossConnections.length > 0) {
    ...
    for (const conn of crossConnections) {
      ...
      html += `<div class="tk-conn ${qClass}"><span class="tk-conn-icon">${conn.quality === 'maha-raja' ? '★★' : '★'}</span> ${conn.desc}</div>`;
    }
    html += '</div></div>';
  } else {
